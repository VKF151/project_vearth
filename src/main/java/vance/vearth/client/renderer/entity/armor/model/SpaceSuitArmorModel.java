...
public class SpaceSuitArmorModel<S extends HumanoidRenderState> extends HumanoidModel<S> {
    public static final ArmorModelSet<ModelLayerLocation> MODEL_LAYERS = new ArmorModelSet<>("helmet", "chestplate", "leggings", "boots").map( s -> new ModelLayerLocation(Identifier.fromNamespaceAndPath(Project_vearth.MOD_ID, "space_suit"), s));
    ...
    private static MeshDefinition createBaseArmorMesh() {
        ...
        PartDefinition head = root.addOrReplaceChild(PartNames.HEAD, CubeListBuilder.create().texOffs(0,0).addBox(-4, -8, -4, 8, 8, 8, new CubeDeformation(0.7F)).texOffs(32,0).addBox(-4, -8, -4, 8, 8, 8, new CubeDeformation(0.6F)), PartPose.ZERO);
        head.addOrReplaceChild(PartNames.HAT, CubeListBuilder.create(), PartPose.ZERO);

        PartDefinition body = root.addOrReplaceChild(PartNames.BODY, CubeListBuilder.create().texOffs(16, 32).addBox(-4, 0, -2, 8, 12, 4, new CubeDeformation(0.7F)).texOffs(16, 16).addBox(-4, 0, -2, 8, 12, 4, new CubeDeformation(0.55F)).texOffs(56, 32).addBox(-4, 0, -2, 8, 12, 4, new CubeDeformation(0.35F)), PartPose.ZERO);
        body.addOrReplaceChild("air_tank", CubeListBuilder.create().texOffs(56, 16).addBox(-4, -1, 2, 8, 10, 4, new CubeDeformation(0.5F)), PartPose.ZERO);

        root.addOrReplaceChild(PartNames.LEFT_ARM, CubeListBuilder.create().texOffs(40, 16).mirror().addBox(-1, -2, -2, 4, 12, 4, new CubeDeformation(0.25F)).mirror(false).texOffs(40, 32).mirror().addBox(-1, -2, -2, 4, 12, 4, new CubeDeformation(0.7F)).mirror(false), PartPose.offset(5, 2, 0));
        root.addOrReplaceChild(PartNames.RIGHT_ARM, CubeListBuilder.create().texOffs(32, 48).addBox(-3, -2, -2, 4, 12, 4, new CubeDeformation(0.25F)).texOffs(48, 48).addBox(-3, -2, -2, 4, 12, 4, new CubeDeformation(0.7F)), PartPose.offset(-5, 2, 0));
